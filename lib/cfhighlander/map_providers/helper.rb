require 'cfhighlander/map_providers/account_id'
...
module Cfhighlander
  module MapProviders
    class Helper
      ...
      def self.mappings_provider(provider_name, is_legacy = false)
        ...
        begin
          ...
          if e.to_s.include? "uninitialized constant"
            return self.mappings_provider(provider_name, true) unless is_legacy
            return nil
          end
          STDERR.puts(e.to_s)
          raise e
      ...
      def self.mappings_provider_maps(provider_name, config)
        provider = self.mappings_provider(provider_name)
      ...
      def self.mapping_value(component:, provider_name:, value_name:, key_name: nil)
        ...
        provider = self.mappings_provider(provider_name) if provider.nil?
        unless provider.nil?
